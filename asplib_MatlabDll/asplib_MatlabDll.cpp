...
bool                              g_IsSpectrumVisProcessorConfigured = false;
CSpectrumVisProcessor             g_SpectrumVisProcessor;
CSpectrumVisProcessorConfigurator g_SpectrumVisProcessorConfigurator;
...
DLL_EXPORT RET_ERR create_Biquad(uint32 BiquadQuantity)
{
  if(g_BiquadHandle)
  {
    const uint32 maxOldFreqBands = CBiquadFactory::get_maxBiquads(g_BiquadHandle);
...
//#include "asplib_utils/fft/KissFFT/KissFFTProducts.hpp"

// SpectrumVisProcessor functions
DLL_EXPORT RET_ERR create_SpectrumVisProcessor(uint32 FrameSize)
{
  ...
  if (g_SpectrumVisProcessor.Create(g_SpectrumVisProcessorConfigurator, FrameSize) != ASPLIB_ERR_NO_ERROR)
  {
    return ERR_FATAL_ERROR;
  }
  
  g_IsSpectrumVisProcessorConfigured = true;
...
DLL_EXPORT RET_ERR process_SpectrumVisProcessor(single *Signal)
{
  ...
  g_SpectrumVisProcessor.Process(Signal, Signal);
...
{
  g_SpectrumVisProcessor.Destroy();
  g_IsSpectrumVisProcessorConfigured = false;
