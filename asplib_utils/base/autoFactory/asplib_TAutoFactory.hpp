...
#include <map>
#include <string>
#include "asplib_utils/base/autoFactory/asplib_TProductStatistics.hpp"
...
private:
  ...
  typedef int64_t (*ProductStatistic)();
  typedef struct ProductCallbacks_t
  {
    ProductCreateCallback CreateProduct;
    ProductStatistic      GetActiveProducts;
    ProductStatistic      GetCreatedProducts;
    ProductStatistic      GetDestroyedProducts;
  }ProductCallbacks_t;
...
public:
  ...
  static TypeID RegisterProduct(TypeID ProductID, TFmt FmtIn, TFmt FmtOut, ProductCallbacks_t Callbacks)
  {
    ...
    if (iter != GetProductMap().end())
    {
      throw ASPLIB_STRING_EXCEPTION_MODULE_HANDLER("Product ID already registered at this product factory!", "asplib_utils");
    }
